...
class RestrictedToRestrictedNAT:
    ...
    def _punch_hole(self, ports_self, ports_peer):
        """
        Restricted to Restricted NAT hole punching.
        Both sides punch holes by sending packets from each port to the corresponding peer port.
        Once a packet is sent from our port to peer's port, the peer can send back to our port.
        """
        log("Starting Restricted to Restricted NAT hole punching", general_logfile_path=self.log_path)
        ...
        for i in range(len(ports_self)):
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
                    ...
                    # Send to corresponding peer port to open NAT mapping
                    peer_port = ports_peer[i] if i < len(ports_peer) else ports_peer[i % len(ports_peer)]
                    sock.sendto(b'RC-punch', (self.peer_ip, peer_port))
                    log(f"Hole punch sent from {ports_self[i]} to {self.peer_ip}:{peer_port}",
                        LogType.INFO, "Success", self.log_path)
            except Exception as e:
                log(f"Hole punch failed on port {ports_self[i]}: {e}",
                    LogType.WARNING, "Failure", self.log_path)

        log(f"Restricted NAT hole punching complete for {len(ports_self)} ports.", general_logfile_path=self.log_path)
    ...
    def send(self, filepath, chunk_size=8192):
        ...
        time.sleep(2)  # Allow NAT mappings to stabilize
        ...
        with Manager() as manager:
            ...
            try:
                ...
                for i, (chunk_num, chunk_data) in enumerate(chunks):
                    ...
                    peer_port = self.data_ports_peer[sock_index % len(self.data_ports_peer)]
