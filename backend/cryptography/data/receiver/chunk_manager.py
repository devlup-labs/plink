        ...
        if os.path.exists(chunk_logfile_path):
            with open(chunk_logfile_path, 'r') as f:
                try:
                    ...
                    log(f"Invalid JSON in chunk log file, starting over.", log_type=Logtype.ERROR, status="Failure", general_logfile_path=general_logfile_path)
                    pass
    ...
    try:
        ...
        with open(chunk_logfile_path, 'r') as log_file:
            try:
                ...
                log(f"Invalid JSON in chunk log file: {e}", log_type=Logtype.ERROR, status="Failure", general_logfile_path=chunk_output_dir)
                raise
        ...
        with open(final_file_path, 'wb') as final_file:
            for chunk_name, info in chunks:
                ...
                if not chunk_path or not os.path.exists(chunk_path):
                    ...
                    log(f"Chunk file {chunk_path} not found for chunk {chunk_name}", log_type=Logtype.ERROR, status="Failure", general_logfile_path=chunk_output_dir)
                    continue
                with open(chunk_path, 'rb') as chunk_file:
                    final_file.write(chunk_file.read())
                os.remove(chunk_path)
                #log for successfully added chunk
                log(f"Chunk {chunk_name} added to final file {final_file_path}", log_type=Logtype.INFO, status="Success", general_logfile_path=chunk_output_dir)


        #log for successfully joined chunks
        log(f"All chunks successfully joined into {final_file_path}", log_type=Logtype.INFO, status="Success", general_logfile_path=chunk_output_dir)
        return final_file_path

    except Exception as e:
        #log for error in join_chunks
        log(f"Error joining chunks: {e}", log_type=Logtype.ERROR, status="Failure", general_logfile_path=chunk_output_dir)
        raise